...
- 假设到达速率恒定，根据结论[^1]，固定传输速率和错误概率之间的关系由下式
  ...
  $$
  其中$\overrightarrow{\lambda}$矩阵是矩阵$A$的特征值![image-20221112104552185](论文列表.assets\image-20221112104552185.png)
  
- ON-OFF离散马尔可夫：定义两个状态$state_1 = \{OFF:source~ keeps~silent\},state_2=\{ON:arrival~rate~a_i = lr(bits/block)\}$，其中$r$为到达恒定速率，转移概率矩阵为
  $$
  G = 
  \begin{bmatrix}
  p_{11} & p_{12} \\
...
<img src="论文列表.assets\image-20221112121956977.png" alt="image-20221112121956977" style="zoom: 80%;" />

<img src="论文列表.assets\image-20221112122040753.png" alt="image-20221112122040753" style="zoom:80%;" />
...
- 定理1：由[^3]，式(20)收敛于一个等价的高斯随机变量$\mathcal{Y} \sim \mathcal{N}(\mu,\sigma^2)$，其中![image-20221112135040089](论文列表.assets\image-20221112135040089.png)
  ...
  $$
  ![image-20221112141622390](论文列表.assets\image-20221112141622390.png)

- 当$N_r,N_t$逐渐增大时，由(19)、(24)和(24)上面的等式可得

  ![image-20221112153443066](论文列表.assets\image-20221112153443066.png)
...
- 为了减少支持数据重传所需的缓冲量，成本和延迟敏感的机器类型设备来说是一个非常重要的限制，取$L \in \{50,200 \},M = 2$，如下图展示了中断概率$\Phi_M$和平均消耗能量$\bar \xi$的比较。
  ...
  ![image-20221117175220580](论文列表.assets\image-20221117175220580.png)

  可知$IR-HARQ$具有最佳性能，$\bar \xi$在对数尺度变化是$\ln (200/50)\approx 1.4$

#### 功率分配

- 下图展示了$M$次传输尝试的功率分配与$L=50$时的平均消耗能量$\bar \xi$<img src="论文列表.assets\image-20221117180443716.png" alt="image-20221117180443716"  />
...
<img src="论文列表.assets\image-20221117182205950.png" alt="image-20221117182205950" style="zoom:50%;" />

#### 能效

<img src="论文列表.assets\image-20221117182359802.png" alt="image-20221117182359802" style="zoom:50%;" />
